...
export default function createUserManager(context) {
    ...
    const UserManager = () => {
        ...
        const totalUserPages = Math.max(1, Math.ceil(filteredUsers.length / TABLE_PAGE_SIZE));
        const safeUserPage = Math.min(Math.max(Number(userPage) || 1, 1), totalUserPages);
        const paginatedUsers = filteredUsers.slice(
            (safeUserPage - 1) * TABLE_PAGE_SIZE,
            safeUserPage * TABLE_PAGE_SIZE
        );
        ...
        }, [safeUserPage, setUserPage, userPage]);
        ...
        return (
            <div className="space-y-6">
                                        ...
                                        <tr key={user.id} className={`transition-colors ${userIsActive ? 'hover:bg-slate-50' : 'bg-white hover:bg-slate-50'}`}>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold shrink-0 ${userIsActive ? 'bg-[#C1986E]/10 text-[#C1986E]' : 'inactive-user-avatar bg-slate-200 text-slate-500'}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center justify-center min-w-[112px] text-xs px-3 py-1.5 rounded-full font-medium ${userIsActive ? (normalizeUserRole(user.role) === 'Super Admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700') : 'inactive-user-role-badge bg-slate-200 text-slate-600'}`}>
                                                ...
                                                <div className="flex items-center justify-center gap-2">
                                                    <Tooltip text={userIsActive ? "Nonaktifkan Akses" : "Aktifkan Akses"} position="top">
                                                        <ToggleSwitch
                                                            ...
                                                            disabled={isPending}
                                                            className={userIsActive ? '' : 'inactive-user-switch'}
                                                        />
                                                    </Tooltip>
                ...
                {/* Modal Ubah Password */}
                {isPasswordModalOpen && renderModalInBody(
                    <div
                    ...
                    >
                        <div
                            className="admin-modal-panel bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col"
                        ...
                        >
                            ...
                            <form onSubmit={handleSavePassword} className="flex flex-col">
                                <div className="admin-modal-body p-6 space-y-4">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                className="admin-theme-control w-full border border-slate-200 rounded-lg px-4 pr-11 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] text-sm"
                ...
                {isUserModalOpen && renderModalInBody(
                    <div
                        className="admin-modal-backdrop fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
                                ...
                                <div className="admin-modal-body p-6 space-y-4">
                                    <div className="space-y-1.5">
                                        ...
                                        <input
                                            ...
                                            className="admin-theme-control admin-input-standard-autofill w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] text-sm"
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select
                                            className="admin-theme-control w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] text-sm bg-white"
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="admin-theme-control admin-input-standard-autofill w-full border border-slate-200 rounded-lg px-4 pr-11 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] text-sm"
