...
export default function createProductManager(context) {
    ...
    const ProductManager = () => {
        ...
        const totalProductPages = Math.max(1, Math.ceil(filteredProducts.length / TABLE_PAGE_SIZE));
        const safeProductPage = Math.min(Math.max(Number(productPage) || 1, 1), totalProductPages);
        const paginatedProducts = filteredProducts.slice(
            (safeProductPage - 1) * TABLE_PAGE_SIZE,
            safeProductPage * TABLE_PAGE_SIZE
        );
        ...
        }, [productPage, safeProductPage, setProductPage]);
