...
const BrandManager = () => {
    ...
    const totalBrandPages = Math.max(1, Math.ceil(filteredBrands.length / TABLE_PAGE_SIZE));
    const safeBrandPage = Math.min(Math.max(Number(brandPage) || 1, 1), totalBrandPages);
    const paginatedBrands = filteredBrands.slice(
        (safeBrandPage - 1) * TABLE_PAGE_SIZE,
        safeBrandPage * TABLE_PAGE_SIZE
    );
    ...
    React.useEffect(() => {
        if (brandPage !== safeBrandPage) {
            setBrandPage(safeBrandPage);
        }
    }, [brandPage, safeBrandPage, setBrandPage]);
    ...
    return (
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                {previewBrand && (
                    <div
                        className="admin-modal-backdrop fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
