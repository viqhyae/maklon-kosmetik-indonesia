...
export default function createTagGenerator(context) {
    ...
    const TagGenerator = () => {
        ...
        const totalBatchPages = Math.max(1, Math.ceil(filteredBatches.length / TABLE_PAGE_SIZE));
        const safeBatchPage = Math.min(Math.max(Number(batchPage) || 1, 1), totalBatchPages);
        const paginatedBatches = filteredBatches.slice(
            (safeBatchPage - 1) * TABLE_PAGE_SIZE,
            safeBatchPage * TABLE_PAGE_SIZE
        );
        ...
        React.useEffect(() => {
            if (batchPage !== safeBatchPage) {
                setBatchPage(safeBatchPage);
            }
        }, [batchPage, safeBatchPage, setBatchPage]);
        ...
        return (
            <div className="space-y-6 animate-in fade-in duration-500">
                                    ...
                                    <tr key={batch.id} className={`transition-colors ${batch.status === 'Suspended' ? 'bg-slate-50' : 'hover:bg-slate-50'} ${isPending ? 'opacity-60' : ''}`}>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2 mb-1">
                                                <p className="font-mono text-sm font-semibold text-slate-800">{batch.id}</p>
                                            </div>
                ...
                {typeof document !== 'undefined' && suspendReasonModal.isOpen && createPortal(
                    <div
                        className="admin-modal-backdrop fixed inset-0 z-[320] bg-black/50 backdrop-blur-md"
                        onClick={closeSuspendReasonModal}
                    >
                        <div className="h-full w-full overflow-y-auto">
                            <div className="min-h-full flex items-center justify-center p-4">
                                <div
                                    className="admin-modal-panel bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[96vh] max-h-[calc(100dvh-2rem)]"
                                    onClick={(event) => event.stopPropagation()}
                                >
                                    <div className="admin-modal-header bg-slate-50 border-b border-slate-100 p-5">
                                    ...
                                    <div className="admin-modal-body p-5 space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <textarea
                                                ...
                                                className="admin-theme-control w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] text-sm resize-y min-h-[110px] text-left"
                ...
                {selectedBatchDetail && (
                    <div
                        className="admin-modal-backdrop fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
                        onClick={() => setSelectedBatchDetail(null)}
                    >
                        <div
                            className="admin-modal-panel bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]"
                            onClick={(event) => event.stopPropagation()}
                        >
                            <div className="admin-modal-header bg-slate-50 border-b border-slate-100 p-4 px-6 flex justify-between items-center z-10 sticky top-0">
                            ...
                            <div className="admin-modal-footer bg-slate-50 border-t border-slate-100 p-4 px-6 flex justify-end z-10 sticky bottom-0">
