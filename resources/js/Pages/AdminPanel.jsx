...
import createUserManager from './AdminPanel/Views/UserManager';
import createScanHistory from './AdminPanel/Views/ScanHistory';
...
const DEFAULT_BRAND_SORT = { key: 'name', direction: 'asc' };
const DEFAULT_PRODUCT_SORT = { key: 'name', direction: 'asc' };
const DEFAULT_USER_SORT = { key: 'name', direction: 'asc' };
const DEFAULT_BATCH_SORT = { key: 'product', direction: 'asc' };
const DEFAULT_SCAN_SORT = { key: 'time', direction: 'desc' };
...
export default function AdminPanel({
    ...
    useEffect(() => {
        ...
        document.body.classList.toggle('admin-theme-dark', isDarkMode);
    ...
    const [brandSort, setBrandSort] = useState(DEFAULT_BRAND_SORT);
    const [productSort, setProductSort] = useState(DEFAULT_PRODUCT_SORT);
    const [userSort, setUserSort] = useState(DEFAULT_USER_SORT);
    const [batchSort, setBatchSort] = useState(DEFAULT_BATCH_SORT);
    const [scanSort, setScanSort] = useState(DEFAULT_SCAN_SORT);
    ...
    useEffect(() => {
        setGlobalSearch('');
        if (activeTab === 'brand') {
            setBrandSort(DEFAULT_BRAND_SORT);
            setBrandPage(1);
        }
        ...
        if (activeTab === 'tags') {
            setBatchSort(DEFAULT_BATCH_SORT);
            setBatchPage(1);
        }
        if (activeTab === 'users') {
        ...
        if (activeTab === 'scan_history') setScanSort(DEFAULT_SCAN_SORT);
    }, [activeTab]);

    useEffect(() => {
        setBrandPage(1);
    }, [brandSort.direction, brandSort.key, globalSearch]);

    useEffect(() => {
        setProductPage(1);
    }, [globalSearch, productSort.direction, productSort.key]);

    useEffect(() => {
        setBatchPage(1);
    }, [batchSort.direction, batchSort.key, globalSearch]);

    useEffect(() => {
        setUserPage(1);
    }, [globalSearch, userSort.direction, userSort.key]);
    ...
    useEffect(() => {
        const shouldLiveSyncScanLogs = activeTab === 'scan_history';
    ...
    // --- UI COMPONENTS ---
    const userManagerContextRef = useRef(null);
    userManagerContextRef.current = {
    ...
    const stableUserManagerRef = useRef(null);
    if (!stableUserManagerRef.current) {
        stableUserManagerRef.current = createUserManager(() => userManagerContextRef.current);
    }
    const StableUserManager = stableUserManagerRef.current;

    const scanHistoryContextRef = useRef(null);
    scanHistoryContextRef.current = {
        globalSearch,
        handleSortChange,
        PageAlert,
        scanLogs,
    ...
    const stableScanHistoryRef = useRef(null);
    if (!stableScanHistoryRef.current) {
        stableScanHistoryRef.current = createScanHistory(() => scanHistoryContextRef.current);
    }
    const StableScanHistory = stableScanHistoryRef.current;
    ...
    return (
        <div className={`h-screen w-full flex font-sans overflow-hidden ${isDarkMode ? 'admin-theme-dark bg-slate-950 text-slate-100' : 'bg-[#F8F9FA] text-slate-800'}`}>
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
                ...
                >
                    <div className="max-w-7xl mx-auto">
                        ...
                        {activeTab === 'users' && <StableUserManager />}
                        {activeTab === 'scan_history' && <StableScanHistory />}
            ...
            {isBrandModalOpen && (
                <div
                    className="admin-modal-backdrop fixed inset-0 z-[400] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
                    onClick={() => { if (!isSavingBrand) handleCancelEditBrand(); }}
                >
                    <div
                        className="admin-modal-panel bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]"
                        onClick={(event) => event.stopPropagation()}
                    >
                        <div className="admin-modal-header bg-slate-50 border-b border-slate-100 p-4 px-6 flex justify-between items-center">
                            ...
                            <div className="admin-modal-body p-6 overflow-y-auto custom-scrollbar">
                                    ...
                                    <div className="flex-1 w-full space-y-4">
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                ...
                                                className="admin-theme-control w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] transition-shadow text-sm"
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <select
                                                className="admin-theme-control w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] bg-white text-sm"
                                ...
                                <div className="mt-4 space-y-1.5">
                                    ...
                                    <textarea
                                        ...
                                        className="admin-theme-control w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C1986E] transition-shadow resize-none text-sm"
                                        value={brandInput.description}
                                        onChange={(event) => setBrandInput({ ...brandInput, description: event.target.value })}
                                    />
                                </div>
                            </div>
                            <div className="admin-modal-footer bg-slate-50 border-t border-slate-100 p-4 px-6 flex justify-end gap-3">
                                <button
                                    type="button"
                                    disabled={isSavingBrand}
                                    onClick={handleCancelEditBrand}
        ...
        .admin-theme-control::placeholder {
        ...
        .admin-theme-dark .inactive-user-avatar {
          background-color: #475569 !important;
          color: #cbd5e1 !important;
        }
        .admin-theme-dark .inactive-user-role-badge,
        .admin-theme-dark .inactive-user-switch {
          background-color: #475569 !important;
          color: #cbd5e1 !important;
        }
        ...
        .admin-theme-dark .admin-modal-panel {
          background-color: #0f172a !important;
          border-color: #334155 !important;
          color: #e2e8f0 !important;
        }
        .admin-theme-dark .admin-modal-header,
        .admin-theme-dark .admin-modal-footer {
          background-color: #111827 !important;
          border-color: #334155 !important;
        }
        .admin-theme-dark .admin-modal-body {
          background-color: #0f172a !important;
        }
        .admin-theme-dark .admin-theme-control {
          background-color: #0b1220 !important;
          color: #e2e8f0 !important;
          border-color: #334155 !important;
        }
        .admin-theme-dark .admin-theme-control::placeholder {
          color: #64748b !important;
        }
        .admin-theme-dark .admin-theme-control option {
          background-color: #0b1220 !important;
          color: #e2e8f0 !important;
        }
        ...
        .admin-input-standard-autofill:-webkit-autofill,
        .admin-input-standard-autofill:-webkit-autofill:hover,
        .admin-input-standard-autofill:-webkit-autofill:focus,
        .admin-input-standard-autofill:-webkit-autofill:active {
        ...
        .admin-input-standard-autofill:-internal-autofill-selected {
          -webkit-text-fill-color: #0f172a !important;
          caret-color: #0f172a !important;
          -webkit-box-shadow: 0 0 0 1000px #ffffff inset !important;
          box-shadow: 0 0 0 1000px #ffffff inset !important;
        ...
        .admin-theme-dark .admin-input-standard-autofill:-webkit-autofill,
        .admin-theme-dark .admin-input-standard-autofill:-webkit-autofill:hover,
        .admin-theme-dark .admin-input-standard-autofill:-webkit-autofill:focus,
        .admin-theme-dark .admin-input-standard-autofill:-webkit-autofill:active {
        ...
        .admin-theme-dark .admin-input-standard-autofill:-internal-autofill-selected {
          -webkit-text-fill-color: #e2e8f0 !important;
          caret-color: #e2e8f0 !important;
          -webkit-box-shadow: 0 0 0 1000px #0b1220 inset !important;
          box-shadow: 0 0 0 1000px #0b1220 inset !important;
      ...
      `}} />
            ...
            {confirmObj.isOpen && (
                <div
                    className="admin-modal-backdrop fixed inset-0 z-[300] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
                    onClick={() => setConfirmObj({ ...confirmObj, isOpen: false })} // Fungsi tutup saat klik background
                >
                    <div
                        className="admin-modal-panel bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col text-center"
                    ...
                    >
                        ...
                        <div className="admin-modal-footer p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
